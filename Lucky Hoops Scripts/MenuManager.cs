...
public class MenuManager : MonoBehaviour
{
    ...
    public void NewGame()
    {
        SceneManager.LoadScene("Present Selection");
    }
    public void SetBoardToLoad(string boardName)
    {
        boardToLoad = boardName;
    }
    public void LoadAssignedBoard()
    {
        ScoinManager.instance.SubtractScoins(giftRoundCost);
        SceneManager.LoadScene(boardToLoad);
    }

    public void FlipGiftMovementState()
    {
        giftMovementPlaying = !giftMovementPlaying;

        for (int i = 0; i < gifts.Length; i++)
        {
            gifts[i].enabled = giftMovementPlaying;
        }
    }
    ...
    {
        if (SceneManager.GetActiveScene().name == "Present Selection")
        {
